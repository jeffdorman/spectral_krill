...
def extract_latlon_data(root):
    ...
    for x in root.iter('ULLON'):
        UL_lon.append(x.text)
    UL_lon=float(UL_lon[0])

    for x in root.iter('ULLAT'):
        UL_lat.append(x.text)
    UL_lat=float(UL_lat[0])

    for x in root.iter('ULHAE'):
        UL_hae.append(x.text)
    UL_hae=float(UL_hae[0])

    UR_lon=[]
    UR_lat=[]
    UR_hae=[]

    for x in root.iter('URLON'):
        UR_lon.append(x.text)
    UR_lon=float(UR_lon[0])

    for x in root.iter('URLAT'):
        UR_lat.append(x.text)
    UR_lat=float(UR_lat[0])

    for x in root.iter('URHAE'):
        UR_hae.append(x.text)
    UR_hae=float(UR_hae[0])

    LL_lon=[]
    LL_lat=[]
    LL_hae=[]

    for x in root.iter('LLLON'):
        LL_lon.append(x.text)
    LL_lon=float(LL_lon[0])

    for x in root.iter('LLLAT'):
        LL_lat.append(x.text)
    LL_lat=float(LL_lat[0])

    for x in root.iter('LLHAE'):
        LL_hae.append(x.text)
    LL_hae=float(LL_hae[0])

    LR_lon=[]
    LR_lat=[]
    LR_hae=[]

    for x in root.iter('LRLON'):
        LR_lon.append(x.text)
    LR_lon=float(LR_lon[0])

    for x in root.iter('LRLAT'):
        LR_lat.append(x.text)
    LR_lat=float(LR_lat[0])

    for x in root.iter('LRHAE'):
        LR_hae.append(x.text)
    LR_hae=float(LR_hae[0])
    
    return UL_lon, UL_lat, UL_hae, UR_lon, UR_lat, UR_hae,            LL_lon, LL_lat, LL_hae, LR_lon, LR_lat, LR_hae
